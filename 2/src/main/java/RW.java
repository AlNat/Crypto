...
class RW {
    ...
    private void Eratosfen (boolean numbers[]) {
        ...
        for (int t = 2; t * t < numbers.length; t++) { // Прошли по всему массиву
            ...
            if (numbers[t]) { // Если это число простое
                for (int t2 = t * t; t2 < numbers.length; t2 += t) { // То находим числа кратные ему, начиная с t * t
                    numbers[t2] = false; // И помечаем, что они составные
                }
            }
